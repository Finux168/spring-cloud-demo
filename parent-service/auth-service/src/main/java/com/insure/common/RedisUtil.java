...
public class RedisUtil {
	 ...
	 * @return
	 */
	public Object hmulitGet0(String key,Object field){
		...
		try {
			...
			hashSet.add(field);
			List<Object> values = hmultiGet(key, hashSet);
			if (values.size() > 0){
				return values.get(0);
			}else {
				return null;
	 ...
	 * @return
	 */
	public List<Object> hmulitGetBatch0(Set<String> keys,Object field){
		if (CollectionUtils.isEmpty(keys) || field == null){
			return null;
		}
		try {
			return keys.stream().map(key->hmulitGet0(key,field)).filter(item->item!=null).collect(Collectors.toList());
		}catch (Exception e){
			e.printStackTrace();
			return null;
		}
